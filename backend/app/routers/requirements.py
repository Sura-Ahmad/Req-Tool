...
def update_requirement(requirement_id: str, data: UpdateRequirementRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return requirement_service.update_requirement(requirement_id, data.description, db)


@router.delete("/{requirement_id}")
def delete_requirement(requirement_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return requirement_service.delete_requirement(requirement_id, db)


@router.post("/add")
def add_requirement(data: AddRequirementRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
