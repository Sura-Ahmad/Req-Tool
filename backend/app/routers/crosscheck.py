...
def cross_check(request: Request, session_id: UUID, db: Session = Depends(get_db)):
    issues, ambiguities, duplicates, inconsistencies = crosscheck_service.run(str(session_id), db)
